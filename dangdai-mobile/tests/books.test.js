System.register(["@playwright/test"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
             ...
             */
            test_1.test.describe('Book Selection Flow', () => {
                ...
                test_1.test('books screen renders when authenticated', async ({ page }) => {
                    ...
                    if (loginVisible) {
                        ...
                        test_1.expect(loginVisible).toBe(true);
                    }
                    else {
                        ...
                        await test_1.expect(page.getByTestId('books-header')).toHaveText('Books');
                        // Verify all 4 books are displayed
                        await test_1.expect(page.getByTestId('book-card-1')).toBeVisible();
                        await test_1.expect(page.getByTestId('book-card-2')).toBeVisible();
                        await test_1.expect(page.getByTestId('book-card-3')).toBeVisible();
                        await test_1.expect(page.getByTestId('book-card-4')).toBeVisible();
                    }
                });
                test_1.test('book data constants are correctly defined', async ({ page }) => {
                    ...
                    test_1.expect(pageLoaded).toBe(true);
                });
             ...
             */
            test_1.test.describe('Book Selection Flow (Authenticated)', () => {
                ...
                test_1.test('books screen displays all four books', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                ...
                test_1.test('book cards display correct titles and Chinese names', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                    // Verify book titles
                    await test_1.expect(page.getByTestId('book-title-1')).toHaveText('Book 1');
                    await test_1.expect(page.getByTestId('book-title-2')).toHaveText('Book 2');
                    await test_1.expect(page.getByTestId('book-title-3')).toHaveText('Book 3');
                    await test_1.expect(page.getByTestId('book-title-4')).toHaveText('Book 4');
                    // Verify Chinese titles
                    await test_1.expect(page.getByTestId('book-title-chinese-1')).toContainText('當代中文課程');
                    await test_1.expect(page.getByTestId('book-title-chinese-2')).toContainText('當代中文課程');
                });
                test_1.test('book cards show progress text for new users', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                    // Progress text should be visible (value depends on user's progress)
                    await test_1.expect(page.getByTestId('book-progress-text-1')).toBeVisible();
                    await test_1.expect(page.getByTestId('book-progress-text-2')).toBeVisible();
                    await test_1.expect(page.getByTestId('book-progress-text-3')).toBeVisible();
                    await test_1.expect(page.getByTestId('book-progress-text-4')).toBeVisible();
                });
                test_1.test('tapping book card navigates to chapter list', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                    ...
                    await test_1.expect(page.getByTestId('chapter-list-header')).toContainText('當代中文課程 第一冊');
                });
                test_1.test('book covers and progress bars are visible', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                    // Verify book covers exist
                    await test_1.expect(page.getByTestId('book-cover-1')).toBeVisible();
                    await test_1.expect(page.getByTestId('book-cover-2')).toBeVisible();
                    // Verify progress bars exist
                    await test_1.expect(page.getByTestId('book-progress-bar-1')).toBeVisible();
                    await test_1.expect(page.getByTestId('book-progress-bar-2')).toBeVisible();
                });
                test_1.test('can navigate to all books', async ({ page }) => {
                    if (!TEST_EMAIL || !TEST_PASSWORD)
                        return;
                    await page.goto('/(tabs)/books', { waitUntil: 'networkidle' });
                    await test_1.expect(page.getByTestId('books-screen')).toBeVisible({ timeout: 10000 });
                    ...
                    await test_1.expect(page.getByTestId('chapter-list-screen')).toBeVisible({ timeout: 5000 });
                    await test_1.expect(page.getByTestId('chapter-list-header')).toContainText('第三冊');
                    ...
                    await test_1.expect(page.getByTestId('chapter-list-screen')).toBeVisible({ timeout: 5000 });
                    await test_1.expect(page.getByTestId('chapter-list-header')).toContainText('第四冊');
                });
            });
        }
    };
