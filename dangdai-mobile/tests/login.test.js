    ...
    return {
        ...
        execute: function () {
            test_1.test.describe('Login Flow', () => {
                ...
                test_1.test('login screen renders with all required elements', async ({ page }) => {
                    // Wait for the page to load
                    await test_1.expect(page.getByRole('heading', { name: 'Sign In' })).toBeVisible({ timeout: 10000 });
                    await test_1.expect(page.getByText('Welcome back to your Chinese learning journey')).toBeVisible();
                    ...
                    await test_1.expect(page.getByText('Forgot Password?')).toBeVisible();
                });
                ...
                test_1.test('navigates to signup screen when clicking sign up link', async ({ page }) => {
                    await test_1.expect(page.getByRole('heading', { name: 'Sign In' })).toBeVisible({ timeout: 10000 });
                ...
                test_1.test('form accepts valid input and attempts submission', async ({ page }) => {
                    ...
                    await test_1.expect(page.getByText('Please enter a valid email')).not.toBeVisible();
                    await test_1.expect(page.getByText('Email is required')).not.toBeVisible();
                    await test_1.expect(page.getByText('Password is required')).not.toBeVisible();
                ...
                test_1.test('shows error message for invalid credentials', async ({ page }) => {
                    await test_1.expect(page.getByRole('heading', { name: 'Sign In' })).toBeVisible({ timeout: 10000 });
                    ...
                    await test_1.expect(page.getByText('Invalid email or password')).toBeVisible({ timeout: 15000 });
                });
            });
        }
    };
