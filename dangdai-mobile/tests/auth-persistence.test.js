System.register(["@playwright/test"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            test_1.test.describe('Auth State Persistence (Story 2.6)', () => {
                test_1.test.describe('AC #4: Loading/Splash Screen', () => {
                    test_1.test('shows loading state while determining auth state', async ({ page }) => {
                        ...
                        await test_1.expect(loginHeading.or(dashboardHeading)).toBeVisible({ timeout: 10000 });
                    });
                });
                test_1.test.describe('AC #3: User data available from auth context', () => {
                    test_1.test('auth context provides user data after sign in', async ({ page }) => {
                        // Navigate to login
                        ...
                        const dashboardHeading = page.getByRole('heading', { name: 'Dashboard' });
                        await test_1.expect(errorMessage.or(dashboardHeading)).toBeVisible({ timeout: 15000 });
                    });
                });
                test_1.test.describe('AC #1 & #2: Route Protection', () => {
                    test_1.test('unauthenticated users are redirected to login', async ({ page }) => {
                        // Try to access protected route directly
                        await page.goto('/(tabs)', { waitUntil: 'networkidle' });
                        // Should redirect to login screen
                        await test_1.expect(page.getByRole('heading', { name: 'Sign In' })).toBeVisible({ timeout: 10000 });
                    });
                    test_1.test('login screen is accessible for unauthenticated users', async ({ page }) => {
                        await page.goto('/(auth)/login', { waitUntil: 'networkidle' });
                ...
                test_1.test.describe('SplashScreen Component', () => {
                    test_1.test('splash screen renders with spinner and loading text', async ({ page }) => {
                        ...
                        await test_1.expect(loadingText.or(loginHeading)).toBeVisible({ timeout: 10000 });
                    });
                });
                test_1.test.describe('Session Expiry Handling (AC #2)', () => {
                    test_1.test('login screen shows after session is cleared', async ({ page }) => {
                        ...
                        await test_1.expect(page.getByRole('button', { name: 'Sign In' })).toBeEnabled();
                    });
                });
            });
        }
