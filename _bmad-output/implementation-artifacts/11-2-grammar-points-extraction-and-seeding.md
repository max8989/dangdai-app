...
- [ ] Task 1: Create grammar chunk parser (AC: #1, #2)
  - [ ] 1.1 Create `dangdai-api/src/scripts/seed_grammar_points.py`
  - [ ] 1.2 Load `book{1-4}_chunks.json` and filter chunks where `metadata.section == 'grammar'`
  - [ ] 1.3 Parse grammar chunk content to extract individual grammar points — each chunk typically contains multiple grammar rules separated by headings (e.g., "I. Ways to Ask Questions", "II. The Particle 呢")
  - [ ] 1.4 For each grammar point, extract: title (English + Chinese), function description, structure pattern, usage notes, examples with traditional/pinyin/english
  - [ ] 1.5 Handle OCR noise in chunk content: common issues include garbled characters, missing tones, mixed-in page numbers

- [ ] Task 2: Implement LLM-assisted extraction (AC: #2, #4)
  - [ ] 2.1 Use OpenAI/Azure OpenAI to parse messy grammar chunk text into structured JSON
  - [ ] 2.2 Prompt the LLM with the raw chunk content and a clear output schema matching the `grammar_points` table fields
  - [ ] 2.3 Validate LLM output against expected schema (title_english required, examples must be array)
  - [ ] 2.4 Fall back to PDF extraction for chunks with `content_quality < 0.85` or where LLM extraction fails
  - [ ] 2.5 Add `--use-pdf` flag to supplement from PDFs at `/home/maxime/Documents/NTNU Book/`

- [ ] Task 3: Create Supabase upsert logic (AC: #5, #6)
  - [ ] 3.1 Use `get_supabase_client()` from `src.utils.supabase` (service key bypasses RLS)
  - [ ] 3.2 Batch upsert using `.upsert()` on the UNIQUE constraint from Story 1.10
  - [ ] 3.3 Assign `grammar_order` sequentially per lesson (1, 2, 3...) and `sort_order` globally across the lesson
  - [ ] 3.4 Log progress: book/lesson transitions, point counts, extraction warnings

- [ ] Task 4: Create CLI entry point (AC: #1)
  - [ ] 4.1 Add `if __name__ == "__main__":` block with `--chunks-dir` (default: `dangdai-rag/output_chunks/`)
  - [ ] 4.2 Add `--dry-run` flag that parses and validates without inserting
  - [ ] 4.3 Add `--book` flag to process a single book (useful for debugging)
  - [ ] 4.4 Print summary: grammar points per book per lesson, OCR quality warnings

- [ ] Task 5: Write unit tests (AC: #2, #5)
  - [ ] 5.1 Create `dangdai-api/tests/unit_tests/test_seed_grammar_points.py`
  - [ ] 5.2 Test chunk filtering by section type
  - [ ] 5.3 Test grammar point extraction from sample chunk content (provide inline test fixtures)
  - [ ] 5.4 Test grammar_order assignment (sequential per lesson)
  - [ ] 5.5 Test handling of malformed chunk content (missing fields, OCR noise)

- [ ] Task 6: Run seeding and verify (AC: #3, #7)
  ...
  - [ ] 6.5 Run `make test` to ensure no regressions
