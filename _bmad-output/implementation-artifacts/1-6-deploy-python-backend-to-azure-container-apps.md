...
- [x] Task 6: Deploy and verify (AC: #4, #5)
  - [x] 6.1 Run `terraform init`
  - [x] 6.2 Run `terraform plan` and review
  - [x] 6.3 Run `terraform apply`
  - [x] 6.4 Test `/health` endpoint from public URL
  - [x] 6.5 Update mobile app with production API URL
...
- **2026-02-15**: All tasks completed - Full Azure deployment successful
- Created terraform/ directory with main.tf, variables.tf, outputs.tf, terraform.tfvars.example, and .gitignore
- Added Azure Container Registry (ACR) to Terraform configuration for container image hosting
- Terraform resources created:
  - Azure Resource Group: dangdai-rg
  - Azure Container Registry: dangdaiacr.azurecr.io
  - Log Analytics Workspace: dangdai-logs
  - Container Apps Environment: dangdai-env
  - Container App: dangdai-api with scale-to-zero (0-10 replicas), 0.5 vCPU, 1GB memory
...
- Docker image built and pushed to ACR using `az acr build`: dangdaiacr.azurecr.io/dangdai-api:v1.0.0
- API deployed and accessible at: https://dangdai-api.yellowcoast-f84a7cfb.eastus.azurecontainerapps.io
- Health endpoint verified: `curl /health` returns `{"status":"healthy"}`
- Mobile app .env.example updated with production API URL
...
- terraform/terraform.tfvars (new - gitignored, contains secrets)
- terraform/.gitignore (new)
- terraform/.terraform.lock.hcl (new - generated by terraform init)
