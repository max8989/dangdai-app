...
export const Route = createFileRoute('/_authed/quiz/play')({
  component: PlayPage,
})
...
function getPrimaryContent(question: QuizQuestion, displayVariant: QuizDisplayVariant): string {
  if (displayVariant === 'character' && question.character) return question.character
  if (displayVariant === 'pinyin' && question.pinyin) return question.pinyin
...
function getTextInputQuestionType(question: QuizQuestion): 'pinyin' | 'meaning' {
  if (question.question_text.toLowerCase().includes('pinyin')) return 'pinyin'
  return 'meaning'
}
...
function PlayPage() {
  ...
  const bookId = useQuizStore((s) => s.bookId)
  ...
  }, [isInvalidQuiz, isIndexOutOfRange, isComplete, hasNavigatedAway])
  ...
  // Phase 4 will add sentence_construction (needs DnD); matching skipped per HANDOFF
  ...
  const isTextInput =
    currentQuestion?.input_type === 'text_input' ||
  ...
  const handleTextInputAnswer = useCallback(
    (userAnswer: string, isCorrect: boolean) => {
      ...
      saveQuestionResult({
        ...
        vocabularyItem: currentQuestion.character ?? null,
        grammarPattern:
          currentQuestion.exercise_type === 'grammar' ? currentQuestion.question_text : null,
        correct: isCorrect,
        timeSpentMs,
      })
  ...
  const handlePause = useCallback(async () => {
    ...
    const currentExerciseType =
      storeState.exerciseType ?? quizPayload?.exercise_type ?? ''
    ...
    const pausedState: PausedQuizState = {
      ...
      exerciseType: currentExerciseType as ExerciseType,
    ...
    try {
      ...
      toast.success('Quiz paused', {
        description: 'Resume anytime from the dashboard.',
      })
      ...
      toast.error("Can't pause quiz", {
        description: 'No internet connection. Try again.',
      })
    }
  }, [quizPayload, currentQuestionIndex, pauseQuiz, clearResumableQuiz])
  ...
  if (
    (isInvalidQuiz || isIndexOutOfRange || !currentQuestion) &&
    !isComplete
  ) {
    return null
  }
    ...
    <>
      ...
      >
        <header className="flex items-center justify-between px-4 pb-2 pt-safe-3">
          ...
          <p className="text-base font-semibold" data-testid="quiz-title">
            {exerciseTypeLabel}
          </p>
          ...
          {isMatching ? (
            ...
            <div key={currentQuestionIndex} className="space-y-4">
              {currentQuestion.english_translation ? (
                <p className="text-xl font-bold" data-testid="fill-in-blank-english">
                  {currentQuestion.english_translation}
                </p>
              ) : null}
              <p
                className="text-base text-muted-foreground font-medium"
                data-testid="fill-in-blank-instruction"
              >
